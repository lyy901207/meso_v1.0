...
revised at 2019/2/25,added the decision of comparing the saved-time-of-value and additional charged toll
'''
...
from h_OtherFunctions.data_for_box_whisker_plot import *
...
def toll_Spent(dic_lane_sec_id_length):
    # this function returns the toll the vehicle needs to pay when changing to a fast lane.
    # the toll is calculated by kilometers driven after the lane-changing
    # input is a dictionary whose key is a series of lane-ids and value is length of that lane or section left of that lane
    toll=0
    for lane_id in dic_lane_sec_id_length:
        toll+=1.0*dic_lane_sec_id_length[lane_id]*dic_lanes[lane_id]['fixed-charge']   
    return toll
...
def decision_Make(veh_id,curr_timestamp):
    ...
    saved_time=needed_time_low-needed_time_high# the saved time if the vehicle changes to a fast-lane;positive value
    lost_time=left_time_budget-needed_time_low # the time difference if vehicle stays at the low-lane;negative value
    ...
    if left_time_budget<needed_time_low:
        if dic_vehicles[veh_id]['veh-type']=='bus':
            if left_time_budget>=needed_time_high:
                chan_lane=1            
                saved_time_value=1.0*saved_time/3600*30*40# saved time in seconds; assume 30 passengers on bus, average time-of-value is 40 SGD/hour
                needed_additional_toll=0# no additional toll for the bus
                dic_vehicles[veh_id]['lane-changed']=[1,curr_timestamp,saved_time,saved_time_value,needed_additional_toll]
            else:# even change the bus cannot arrive on time
                chan_lane=1
                lost_time=left_time_budget-needed_time_high
                lost_time_value=1.0*lost_time/3600*30*40
                needed_additional_toll=0
                dic_vehicles[veh_id]['lane-changed']=[1,curr_timestamp,lost_time,lost_time_value,needed_additional_toll]
        elif dic_vehicles[veh_id]['veh-type']=='car':
            #####check whether changing to a fast-lane can arrive on time
            if left_time_budget<needed_time_high:
                ...
                lost_time_value=1.0*lost_time*dic_vehicles[veh_id]['value-time']/3600
                dic_vehicles[veh_id]['lane-changed']=[9,curr_timestamp,lost_time,lost_time_value,"NA"]# changing lane still cannot meet the time budget            
            else:#if changing to a fast-lane can make it arrive on time
                #####check the additionally charged toll fee can be accepted by the driving; compare the toll with driver's value-of-time
                # calculate the distance from current position to its destination via high-lane network
                dic_lane_sec_id_length={}# key is the lane-id along the route of high-lanes; key is the length of that lane or left length on that lane (current lane when changing lanes)
                for from_lane_id in best_route_results['best-route-lanes-high']:
                    if not dic_lane_sec_id_length.has_key(from_lane_id):
                        dic_lane_sec_id_length[from_lane_id]=dic_lanes[from_lane_id]['length']
                    to_lane_id=best_route_results['best-route-lanes-high'][from_lane_id]
                    if not dic_lane_sec_id_length.has_key(to_lane_id):            
                        dic_lane_sec_id_length[to_lane_id]=dic_lanes[to_lane_id]['length']
                dic_lane_sec_id_length[lane_id]=cal_Dist(curr_location,dic_nodes[nodeid2])/1000.0
                needed_addtional_toll=toll_Spent(dic_lane_sec_id_length)
                # calculate the saved time-of-value
                saved_time_value=dic_vehicles[veh_id]['value-time']*saved_time/3600.0
                if saved_time_value>=needed_addtional_toll:
                    chan_lane=1             
                    dic_vehicles[veh_id]['lane-changed']=[1,curr_timestamp,saved_time,saved_time_value,needed_addtional_toll]
                else:
                    chan_lane=0
                    lost_time_value=1.0*lost_time*dic_vehicles[veh_id]['value-time']/3600
                    dic_vehicles[veh_id]['lane-changed']=[8,curr_timestamp,lost_time,lost_time_value,needed_addtional_toll]# not change due to that saved time-of-value is less than the toll
...
multi_OD_para=1 # multiple times of basic OD matrices, a parameter to increase the travel demand

# set the charging rate of fast-lane
scenario_test_list_fast_lane_charge=[0.01,0.1,0.3,0.5,0.7,0.9,1,10]#0.0001,0.001, unit in SGD per kilometers, this is the fixed type of charge as it does not vary due to density on lanes
fast_lane_charge=1
...
for multi_OD_para in scenario_test_list_multi_OD_para:
    for fast_lane_charge in scenario_test_list_fast_lane_charge:                
        item_para_combination=[delay_strategy,veh_gen_distri,sigma_veh_time_gen,mu_veh_time_gen,multi_OD_para,fast_lane_charge]
...
# add the date-time in the file of statistic output file
writer_statistic_along_timestamps.writerow(['the result running at '+str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+' are as follows'])
writer_statistic_whole_simulation.writerow(['the result running at '+str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+' are as follows'])

for item_para_combination in scenario_test_list_para_combinations:
    scenario_count+=1
    print "scenario",scenario_count,"starts; now calculation is running"
    # obtain the parameters
    delay_strategy    =item_para_combination[0]
    veh_gen_distri    =item_para_combination[1]
    sigma_veh_time_gen=item_para_combination[2]
    mu_veh_time_gen   =item_para_combination[3]
    multi_OD_para     =item_para_combination[4]
    fast_lane_charge  =item_para_combination[5]
    
        
    # set the list of heads for output
    list_para_combination_heads=["delay-strategy","veh-generation-method","sigma-norm-distri-veh-gener","demand-times","charge-rate","--"]
    # set the list of parameter-combination for output
    list_para_combination_values=[delay_strategy,veh_gen_distri,sigma_veh_time_gen,multi_OD_para,fast_lane_charge,"--"]
    ...
    dic_lanes=fixed_Charge_Update2(dic_lanes, fast_lane_charge)
    #*************************************************************************#
    ...
    for i in range(0,total_steps):# each simulation step i    
        ...
        #!print "this is ", i, "th step in simulation at time-stamp of ", curr_timestamp, " at scenario",scenario_count#%%%%%%%%%%%% for checking
    ...
    columnheads3=list_para_combination_heads+["veh-id","veh-type","value-time","lane-chan-p","origin","destination","time-budget","start-time","expected-endtime","end-time","lane-changed-status","lane-changed-timestamp","lane-changed-save-or-lost-time","lane-changed-save-time-value","lane-changed-additional-toll"]
    ...
    columnhead_of_vehicles_decision_whole_simu=list_para_combination_heads+["statistic-general","statistic-detail","changed-lane","not-change-lane-as-low-lane-is-enough","not-change-lane-as-no-time-improve","not-change-lane-as-no-save-value"]
    ...
    for veh_id in dic_vehicles:
        if dic_vehicles[veh_id]['veh-type']=='car' and dic_vehicles[veh_id]['lane-changed'][0]==0:
            ...
            sum_car_saved_time_value+=dic_vehicles[veh_id]['lane-changed'][3]
            sum_car_additional_toll+=dic_vehicles[veh_id]['lane-changed'][4]
            aver_speed=sum_Dictionary_Value(dic_vehicles[veh_id]['dic-speeds'])*1.0/len(dic_vehicles[veh_id]['dic-speeds'])
            list_car_1_aver_speed.append(aver_speed)
        elif dic_vehicles[veh_id]['veh-type']=='car' and dic_vehicles[veh_id]['lane-changed'][0]==8:
            count_car_not_changed_no_save+=1            
            sum_car_lost_time_value+=dic_vehicles[veh_id]['lane-changed'][3]
            aver_speed=sum_Dictionary_Value(dic_vehicles[veh_id]['dic-speeds'])*1.0/len(dic_vehicles[veh_id]['dic-speeds'])
            list_car_8_aver_speed.append(aver_speed)
            ...
            sum_bus_saved_time_value+=dic_vehicles[veh_id]['lane-changed'][3]
            aver_speed=sum_Dictionary_Value(dic_vehicles[veh_id]['dic-speeds'])*1.0/len(dic_vehicles[veh_id]['dic-speeds'])
            ...
            sum_bus_lost_time_value+=dic_vehicles[veh_id]['lane-changed'][3]
            aver_speed=sum_Dictionary_Value(dic_vehicles[veh_id]['dic-speeds'])*1.0/len(dic_vehicles[veh_id]['dic-speeds'])
            list_bus_9_aver_speed.append(aver_speed)
        else:
            print "ERROR at categorizing the vehicles regarding the decision-making"
    row_car_decision_count=list_para_combination_values+['decision-made-count','car',count_car_changed_lane,count_car_not_changed,count_car_not_changed_no_improve,count_car_not_changed_no_save]
    row_bus_decision_count=list_para_combination_values+['decision-made-count','bus',count_bus_changed_lane,count_bus_not_changed,count_bus_not_changed_no_improve]
    writer_statistic_whole_simulation.writerow(row_car_decision_count)
    writer_statistic_whole_simulation.writerow(row_bus_decision_count)
    row_car_decision_saved_time_value=list_para_combination_values+['decision-made-saved-time-value','car',sum_car_saved_time_value]
    row_car_decision_lost_time_value =list_para_combination_values+['decision-made-lost-time-value', 'car',sum_car_lost_time_value]
    row_car_decision_additional_toll =list_para_combination_values+['decision-made-additional-toll', 'car',sum_car_additional_toll]
    row_bus_decision_saved_time_value=list_para_combination_values+['decision-made-saved-time-value','bus',sum_bus_saved_time_value]
    row_bus_decision_lost_time_value =list_para_combination_values+['decision-made-lost-time-value', 'bus',sum_bus_lost_time_value]
    writer_statistic_whole_simulation.writerow(row_car_decision_saved_time_value)
    writer_statistic_whole_simulation.writerow(row_car_decision_lost_time_value)
    writer_statistic_whole_simulation.writerow(row_car_decision_additional_toll)
    writer_statistic_whole_simulation.writerow(row_bus_decision_saved_time_value)
    writer_statistic_whole_simulation.writerow(row_bus_decision_lost_time_value)
    
    ######## regarding vehicle's speed by vehicle type and changing lane per simulation
    columnhead_of_vehicles_speed_whole_simu=list_para_combination_heads+["statistic-general","statistic-detail","count-vehicles","aver-speed","min-speed","max-speed","25percentile","50percentile","75percentile","list-extraordinary-values"]
    writer_statistic_whole_simulation.writerow(columnhead_of_vehicles_speed_whole_simu)    
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']
    if len(list_car_0_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_car_0_aver_speed)           
    row_car_0_speeds=list_para_combination_values+["average-speed","car-no-changed-lanes"]+results    
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']
    if len(list_car_1_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_car_1_aver_speed)
    row_car_1_speeds=list_para_combination_values+["average-speed","car-changed-lanes"]+results    
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA'] 
    if len(list_car_8_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_car_8_aver_speed)
    row_car_8_speeds=list_para_combination_values+["average-speed","car-not-changed-lanes-as-no-save"]+results    
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']
    if len(list_car_9_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_car_9_aver_speed)       
    row_car_9_speeds=list_para_combination_values+["average-speed","car-not-changed-lanes-as-no-improve"]+results
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']  
    if len(list_bus_0_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_bus_0_aver_speed)    
    row_bus_0_speeds=list_para_combination_values+["average-speed","bus-no-changed-lanes"]+results
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']
    if len(list_bus_1_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_bus_1_aver_speed)        
    row_bus_1_speeds=list_para_combination_values+["average-speed","bus-changed-lanes"]+results
    
    results=[0,'NA','NA','NA','NA','NA','NA','NA']
    if len(list_bus_9_aver_speed)>0:
        results=data_for_Box_and_Whisker_Plot(list_bus_9_aver_speed)
    row_bus_9_speeds=list_para_combination_values+["average-speed","bus-not-changed-lanes-as-no-improve"]+results
    
    writer_statistic_whole_simulation.writerow(row_car_0_speeds)
    writer_statistic_whole_simulation.writerow(row_car_1_speeds)
    writer_statistic_whole_simulation.writerow(row_car_8_speeds)
    ...
    for veh_id in list_vehids:
        ...
        row=list_para_combination_values+[veh_id,vehicle['veh-type'],vehicle['value-time'],vehicle['lane-chan-p'],vehicle['origin'],vehicle['destination'],vehicle['time-budget'],vehicle['start-timestamp'],vehicle['expected-endtime'],vehicle['end-timestamp'], \
                                          vehicle['lane-changed'][0],vehicle['lane-changed'][1],vehicle['lane-changed'][2],vehicle['lane-changed'][3],vehicle['lane-changed'][4]]
